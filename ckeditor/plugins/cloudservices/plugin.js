...
( function() {
	...
	function get_qiniu_token() {
	  ...
	  xhr.open('POST', window.COMMUNITY_URL_PREFIX + '/api/v1/qiniu/token/', false);

	  xhr.setRequestHeader('Content-Type', 'application/json');

	  let data = {
	    'target': 'article_image',
	    'filename': 'article_image.jpeg',
	  };
	...
	CKEDITOR.plugins.add( 'cloudservices', {
		...
		beforeInit: function( editor ) {
			...
			editor.on( 'fileUploadRequest', function( evt ) {
				...
				qiniu_tokens = get_qiniu_token();

				if (!qiniu_tokens) {
						CKEDITOR.error( 'no-token' );
						evt.cancel();
						return;
				}
				...
				if ( fileLoader instanceof CKEDITOR.plugins.cloudservices.cloudServicesLoader ) {
					...
					reqData.token = qiniu_tokens.upload_token;
					reqData.key = qiniu_tokens.key;
			...
			editor.on( 'fileUploadResponse', function( evt ) {
				...
				if ( fileLoader instanceof CKEDITOR.plugins.cloudservices.cloudServicesLoader ) {
					...
					try {
						evt.data.response = {
							'130': qiniu_tokens.url,
							'210': qiniu_tokens.url,
							'default': qiniu_tokens.url
						};
					} catch ( e ) {
